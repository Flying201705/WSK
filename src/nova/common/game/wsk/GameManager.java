...
public class GameManager implements StageCallBack, GameDispatcher {
	 ...
	 * 测试程序
	 */
	private void updateGameInfoForDebug() {
		if (21 == TestUtil.Debug_Type) {
			mGameRoundInfo.setFirst(1);
	 ...
	 * 测试程序
	 */
	private void updateTributeInfoForDebug() {
		if (TestUtil.Debug_Type == 3) {
			// 测试退贡
			mGameInfo.setTributeCount(2);
			mGameRoundInfo.setCurrent(1);
			mGameRoundInfo.setSecurrent(3);
			updateTributedState(mGameInfo.getTributeCount());
		} else if (TestUtil.Debug_Type == 4) {
			// 测试进贡
			mGameInfo.setTributeCount(2);
			mGameRoundInfo.setCurrent(0);
			mGameRoundInfo.setSecurrent(2);
			updateTributedState(mGameInfo.getTributeCount());
		}
	}
}
